...
export default function ManagerDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 p-6 bg-slate-900 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-slate-400 text-xs font-semibold uppercase tracking-wide">Total Properties</p>
              <h3 className="text-3xl font-bold text-white mt-1">{metrics.totalProperties}</h3>
            </div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-emerald-400 bg-emerald-500/20 px-2 py-1 rounded">+12%</span>
          </div>
        </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-slate-400 text-xs font-semibold uppercase tracking-wide">Active Tenants</p>
              <h3 className="text-3xl font-bold text-white mt-1">{metrics.totalTenants}</h3>
            </div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-emerald-400 bg-emerald-500/20 px-2 py-1 rounded">+5.4%</span>
          </div>
        </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-slate-400 text-xs font-semibold uppercase tracking-wide">Occupancy Rate</p>
              <h3 className="text-3xl font-bold text-white mt-1">{metrics.occupancyRate}%</h3>
            </div>
          ...
          <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
            <div className="h-full bg-cyan-500 rounded-full" style={{ width: `${metrics.occupancyRate}%` }}></div>
          </div>
        </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              <p className="text-slate-400 text-xs font-semibold uppercase tracking-wide">Monthly Revenue</p>
              <h3 className="text-3xl font-bold text-white mt-1">${(metrics.revenue / 1000).toFixed(1)}k</h3>
            </div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-emerald-400 bg-emerald-500/20 px-2 py-1 rounded">+8.1%</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-slate-800 border border-slate-700 rounded-lg p-6">
          <div className="mb-4">
            ...
            <p className="text-xs text-slate-400">2026 Monthly Performance</p>
          </div>
          
          <div className="h-40 flex items-end justify-between gap-1">
            {['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug'].map((month) => {
              const income = Math.floor(Math.random() * 40) + 50;
              const expense = Math.floor(Math.random() * 30) + 20;
              return (
                <div key={month} className="flex-1 flex flex-col items-center gap-1 group">
                  <div className="w-full flex items-end gap-0.5 h-full">
                    <div className="w-1/2 bg-blue-500 rounded-t" style={{ height: `${income}%` }}></div>
                    <div className="w-1/2 bg-purple-500 rounded-t" style={{ height: `${expense}%` }}></div>
                  </div>
                  <span className="text-xs text-slate-500 text-center">{month}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
          <h3 className="text-lg font-bold text-white mb-0.5">Maintenance</h3>
          <p className="text-xs text-slate-400 mb-4">Ticket status</p>

          <div className="flex justify-center mb-6">
            <div className="relative w-32 h-32">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="40" fill="none" className="stroke-slate-700" strokeWidth="10" />
                <circle cx="50" cy="50" r="40" fill="none" className="stroke-emerald-400" strokeWidth="10" strokeDasharray="150.8" strokeDashoffset="0" strokeLinecap="round" />
              </svg>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
        <div className="mb-4">
          ...
          <p className="text-xs text-slate-400">Latest payments and expenses</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700">
                <th className="text-left py-3 font-semibold text-slate-400 text-xs">Transaction</th>
                <th className="text-left py-3 font-semibold text-slate-400 text-xs">Property</th>
                <th className="text-left py-3 font-semibold text-slate-400 text-xs">Date</th>
                <th className="text-left py-3 font-semibold text-slate-400 text-xs">Status</th>
                <th className="text-right py-3 font-semibold text-slate-400 text-xs">Amount</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700">
              {[
                { name: 'Rent Payment', property: 'Downtown Tower, Apt 4B', date: 'Feb 21, 2026', status: 'Completed', amount: '+$2,400', type: 'income' },
                { name: 'Plumbing Repair', property: 'Riverside Complex', date: 'Feb 20, 2026', status: 'Processing', amount: '-$350', type: 'expense' },
                { name: 'Rent Payment', property: 'Westside Heights, Apt 12A', date: 'Feb 19, 2026', status: 'Completed', amount: '+$1,850', type: 'income' },
                { name: 'Security Deposit', property: 'Downtown Tower, Apt 8C', date: 'Feb 18, 2026', status: 'Completed', amount: '+$3,000', type: 'income' },
              ].map((trx, idx) => (
                <tr key={idx} className="hover:bg-slate-700/50 transition-colors">
                  <td className="py-3 font-medium text-white">{trx.name}</td>
                  <td className="py-3 text-slate-300">{trx.property}</td>
                  <td className="py-3 text-slate-400">{trx.date}</td>
                  <td className="py-3">
                    <span className={`text-xs font-semibold px-2 py-1 rounded ${
                      trx.status === 'Completed' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-blue-500/20 text-blue-400'
                    }`}>
                      {trx.status}
                    </span>
                  </td>
                  <td className={`py-3 text-right font-bold ${trx.type === 'income' ? 'text-emerald-400' : 'text-white'}`}>
                    {trx.amount}
                  </td>
                </tr>
              ))}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-4">Overview Dashboard</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="relative group">
          ...
          <div className="relative bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-3xl p-10 sm:p-12 shadow-[0_8px_32px_rgba(0,0,0,0.2)] hover:-translate-y-1 transition-transform duration-300 overflow-hidden">
            ...
            <div>
              <h3 className="text-3xl sm:text-4xl font-bold text-white mb-4">{metrics.totalProperties}</h3>
        ...
        <div className="relative group">
          ...
          <div className="relative bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-3xl p-10 sm:p-12 shadow-[0_8px_32px_rgba(0,0,0,0.2)] hover:-translate-y-1 transition-transform duration-300 overflow-hidden">
            ...
            <div>
              <h3 className="text-3xl sm:text-4xl font-bold text-white mb-4">{metrics.totalTenants}</h3>
              <div className="flex items-center">
        ...
        <div className="relative group">
          ...
          <div className="relative bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-3xl p-10 sm:p-12 shadow-[0_8px_32px_rgba(0,0,0,0.2)] hover:-translate-y-1 transition-transform duration-300 overflow-hidden">
            ...
            <div>
              <h3 className="text-3xl sm:text-4xl font-bold text-white mb-4">{metrics.occupancyRate}%</h3>
        ...
        <div className="relative group">
          ...
          <div className="relative bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-3xl p-10 sm:p-12 shadow-[0_8px_32px_rgba(0,0,0,0.2)] hover:-translate-y-1 transition-transform duration-300 overflow-hidden">
            ...
            <div>
              <h3 className="text-3xl sm:text-4xl font-bold text-white mb-4">${(metrics.revenue / 1000).toFixed(1)}k</h3>
              <div className="flex items-center">
